...
function ChatWindow({ candidate }) {
  ...
  const currentQuestion = candidate.chat[currentQuestionIndex]
  const isInterviewComplete = currentQuestionIndex >= QUESTION_SEQUENCE.length
  ...
  useEffect(() => {
    if (candidate.status === 'created') {
      ...
      setTimeRemaining(candidate.currentTimerRemaining || 0)
      setIsPaused(true)
    } else if (candidate.status === 'in_progress' && !isInterviewComplete) {
      if (!currentQuestion) {
        generateNextQuestion()
      } else if (!currentQuestion.answer && !isActive) {
        // Resume timer for current question
        const timeLimit = QUESTION_SEQUENCE[currentQuestionIndex].timeLimit
        setTimeRemaining(timeLimit)
        setIsActive(true)
      }
    }
  }, [candidate.status, currentQuestionIndex, isInterviewComplete])
  ...
  const startInterviewFlow = async () => {
    dispatch(startInterview({ id: candidate.id }))
    await generateNextQuestion()
  }

  const generateNextQuestion = async () => {
    if (currentQuestionIndex >= QUESTION_SEQUENCE.length) return

    setIsGenerating(true)
    try {
      const questionSpec = QUESTION_SEQUENCE[currentQuestionIndex]
      const questionData = await generateQuestion(questionSpec.difficulty)
      
      dispatch(addQuestion({
        id: candidate.id,
        question: {
          qId: currentQuestionIndex + 1,
  ...
  const handleSubmit = async () => {
    ...
    try {
      const timeTaken = QUESTION_SEQUENCE[currentQuestionIndex].timeLimit - timeRemaining
      ...
      if (currentQuestionIndex + 1 >= QUESTION_SEQUENCE.length) {
        ...
        dispatch(nextQuestion({ id: candidate.id }))
        await generateNextQuestion()
      }
  ...
  const handleResume = () => {
    setIsPaused(false)
    setIsActive(true)
    dispatch(resumeInterview({ id: candidate.id }))
  }
  ...
  if (isInterviewComplete || candidate.status === 'completed') {
    return (
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Previous Questions */}
      {candidate.chat.filter(q => q.answer).map((chatEntry, index) => (
        <Card key={chatEntry.qId} className="bg-gray-50">
          <div className="mb-4">
